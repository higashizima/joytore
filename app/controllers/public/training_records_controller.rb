class Public::TrainingRecordsController < ApplicationController
  ...
  def create
     ...
     if @training_record.image.attached?
      tags = Vision.get_image_data(training_record_params[:image])
     end
     if @training_record.save
       if @training_record.image.attached?
         tags.each do |tag|
          @training_record.tags.create!(name: tag)
         end
       end
  ...
  def training_record_params
    params.require(:training_record).permit(:user_id, :content, :image,
